...
function getAddToCartButton() {
	...
	for (var i = 0; i < buttons.length; i++) {
		...
		if (
			button.textContent.trim() === 'Legg i kurven' ||
...
function getCheckoutButton() {
	...
	for (var i = 0; i < buttons.length; i++) {
		var button = buttons[i];
		if (
			button.textContent.trim() === 'Gå til kassen' ||
			button.textContent.trim() === 'Go to checkout'
		) {
			return button;
		}
	}
...
function isSomethingInCart() {
	var somethingInCart = document.querySelectorAll(
		'.details-product-purchase--in-bag details-product-purchase--add-more details-product-purchase--checkout'
	);
...
async function main() {
	...
	if (isSomethingInCart() && checkoutButton)
	...
	var buttonContainer =
		addToCartButton.parentNode.parentNode.parentNode.parentNode.parentNode
			.parentNode;
	...
	LinkButton.addEventListener('click', function () {
		window.open(
			'https://www.google.com/maps/d/u/0/viewer?mid=1bC6jk4HTcF2ka_d7SNR7EzQ4T0gNUiPl&ll=60.07401596188039%2C10.496927318361355&z=8',
			'_blank'
		);
	});
	...
	inputField.addEventListener('input', function (event) {
		...
		if (isValidZipcode) {
			...
			LinkButton.style.display = 'none';
			if (isSomethingInCart() && checkoutButton)
				checkoutButton.style.marginTop = '0px';
		} else {
			warningParagraph.textContent =
				'Vår leveringspartner kan ikke levere kjølevarer til din adresse helt enda. Klikk på knappen under for å se hvor GroGro er tilgjengelig i butikk';
			addToCartButton.style.display = 'none';
